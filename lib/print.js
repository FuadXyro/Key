...
Message: `)
 // Gausah Tengil Ganti-Ganti Njink Gw Remake Sampe Pusing Ngentod  
  ...
  if ((opts["antibot"] || (global.db.data.settings[conn.user.jid]?.antibot && m?.isGroup)) && m?.sender) {
    const idBot = m.msg?.id || m.key?.id || 'N/A' // tambahkan '?' setelah 'key' agar tidak error jika 'key' tidak terdefinisi
    if (idBot.includes("BAE5") && m.sender !== conn.user.jid) {
        const antiBotMessage = "*A N T I  B O T 𖤶*\n\n> Group ini dilengkapi dengan anti bot\n> Anda melanggar larangan bot\n\n𖤇 Anda berhak di *kick.*"
