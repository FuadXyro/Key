...
const giflogo = "https://telegra.ph/file/6804517dad260596d87e7.mp4"
const logo = "https://telegra.ph/file/6d3ac79c3442c74dee9db.jpg"
...
let handler = async (m, { conn, command, args }) => {
  ...
  if (m.quoted && m.quoted.mtype) {
    ...
    const doc = {}
    const contact = Object.keys(global.db.data.users).length
    
    if (type === 'Audio') {
    	const link = await (type === 'Image' ? uploadImage : uploadFile)(await m.quoted.download())
    ...
    await conn.sendMessage('status@broadcast', doc, {
      ...
      conn.reply(m.chat, `Sukses upload sw ke ${contact} Kontak\nType: ${type}`, res)
    }).catch(() => {
      conn.reply(m.chat, `( ✘ ) Gagal upload sw\n• Type: ${type}`, m)
    })
  } else {
