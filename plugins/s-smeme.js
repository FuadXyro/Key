...
let handler = async (m, {
    ...
    let [atas, bawah] = text.split(/[^\w\s]/g)
        ...
        if (/webp/g.test(mime)) {
            out = await createSticker(meme + await webp2png(img), false, author, name, 60)
        } else if (/image/g.test(mime)) {
            out = await createSticker(meme + await uploadImage(img), false, author, name, 60)
        } else if (/video/g.test(mime)) {
            out = await sticker(meme + await uploadFile(img), false, author, name)
        } else if (/gif/g.test(mime)) {
            out = await createSticker(meme + await uploadFile(img), false, author, name, 60)
