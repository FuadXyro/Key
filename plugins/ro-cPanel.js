...
const linkgc = 'https://chat.whatsapp.com/KIuvRvTUv6KGU7z8BEnE9x' // Group
const domain = 'fuadxy.panelstore.xyz' // Domain Web
const apikey = "ptla_OhsIdksEaMRthHuWPQzTaSNorMMHJi3Obign6yXXmSu"
const c_apikey = "ptlc_CValIsqgBAdsDqI8FJZBHlZMeey0PI2QQjd6RN9hglM" 
const eggs = '6'
const locs = '1'
...
// Time
    ...
    switch (command) {
        case "1gb": {
            ...
            let t = text.split(",");
            if (t.length < 2) return conn.reply(m.chat, `• *Example :* ${_p + command} fall,${nomorown}`, m)
            let nomor = t[1]
            ...
            if (!u) return conn.reply(m.chat, `• *Example :* ${_p + command} fall,${nomorown}`, m)
...
Expired: 1 Bulan`)
        ...
        case "3gb": {
            if (!isROwner) return global.dfail("rowner", m, conn)
            let t = text.split(",");
            if (t.length < 2) throw `• *Example :* ${_p + command} fall,${nomorown}`
            let nomor = t[1]
            ...
            if (!u) throw `• *Example :* ${_p + command} fall,${nomorown}`
            let d = (await conn.onWhatsApp(u.split`@`[0]))[0] || {}
            let profil = d.exists ? crypto.randomBytes(2).toString("hex") : t[2]
            let password = name + "001"
