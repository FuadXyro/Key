...
class SnakeAndLadderGame {
	constructor(sMsg) {
		...
		this.playerImageUrls = {
			red: 'https://cdn.iconscout.com/icon/free/png-512/free-among-us-3187363-2669561.png',
			yellow: 'https://cdn.iconscout.com/icon/free/png-512/free-among-us-3187359-2669557.png'
		};
		this.bgImage = null;
		this.playerImages = {
			yellow: null,
	...
	getPlayerColor = (player) => (player === this.players[0] ? 'yellow' : 'red');
	...
	playTurn = async (m, player) => {
		if (!this.players.length) return m.reply('🛑 *Tidak ada permainan aktif.* Gunakan "!snake start" untuk memulai permainan baru.');
		...
		for (const otherPlayer of this.players) {
			if (otherPlayer !== player && this.currentPositions[otherPlayer] === newPosition) {
				const message = `😱 *Oh tidak!* ${this.formatPlayerName(player)} *diinjak oleh* ${this.formatPlayerName(otherPlayer)}.* Kembali ke awal cell.*`;
		...
		if (newPosition <= this.boardSize) {
			...
			if (checkSnakeOrLadder) {
				...
				await m.reply(`🐍 ${this.formatPlayerName(player)} menemukan ${action === 'Mundur' ? 'ular' : 'tangga'}! ${action} *ke kotak ${checkSnakeOrLadder.end}.*`, null, {
					mentions: this.players
				});
			...
			if (this.currentPositions[player] === this.boardSize) {
				await m.reply(`🎉 ${this.formatPlayerName(player)} menang! Selamat!`, null, {
					mentions: [player]
				});
			...
			if (diceRoll !== 6) {
				...
				await m.reply('🎲 Anda mendapat 6, jadi giliran Anda masih berlanjut.', null, {
					mentions: this.players
				});
			}
		} else {
