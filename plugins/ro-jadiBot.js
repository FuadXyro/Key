...
let rtx= `
*Pindai kode QR ini untuk menjadi Bot (ZenithBotz), Anda dapat menggunakan perangkat lain untuk memindai*
...
*Anda dapat mengirimkan ID yang diberikan ke bot secara pribadi untuk menghubungkan kembali bot tanpa harus memindai kode lagi, kode ini dimulai dengan mengetik .jadibot*
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    async function jddt() {
      ...
      async function connectionUpdate(update) {
        ...
        if (qr) parentw.sendMessage(m.chat, {image: await qrcode.toBuffer(qr, { scale: 8 }) , caption : rtx + `${author}`}, { quoted: m })
